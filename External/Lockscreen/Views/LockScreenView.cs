...
namespace Xamarin.LockScreen.Views
{
    public class LockScreenView : UIView
    {
        ...
        private float CorrectWidth { get { return (float)ContentView.Bounds.Size.Width; } }
        private float CorrectHeight { get { return (float)ContentView.Bounds.Size.Height; } }
        ...
        private bool IsLessThanIOS6
        {
            get
            {
                ...
                Version current = new Version(Constants.Version);
                return current <= ios6;
            }
        }
        ...
        public LockScreenView(CGRect frame) : base(frame)
        {
            SetDefaultStyles();
            var height = frame.Size.Height >= 568.0f ? 568.0f : frame.Size.Height;

            ContentView = new UIView(new RectangleF(0, 0, 320, (float)height));
        ...
        internal void AnimateFailuteNotificationDirection(float direction)
        {
            ...
            UIView.Animate(0.08, animation, success);
        ...
        internal void UpdatePinTextFieldWithLength(int length)
        {
            ...
            if (!IsLessThanIOS6)
            {
                ...
                UIView.Transition(DigitsTextField, AnimationLength,
                    ...
                    new Action(() => DigitsTextField.AttributedText = attrString),
                    null);
            }
            else
            {
        ...
        private void SetDefaultStyles()
        {
            ...
            LabelColor = UIColor.White.FabicColour(Fabic.Data.Enums.FabicColour.Blue); ;
        }
        ...
        private void LayoutTitleArea()
        {
            ...
            float pinSelectionTop = (float)EnterPasscodeLabel.Frame.Y +
                (float)EnterPasscodeLabel.Frame.Size.Height + 17.5f;
            ...
            if (IsComplexPin)
            {
                ...
                OkButton.Frame = new RectangleF(
                    (float)DigitsTextField.Frame.X + (float)DigitsTextField.Frame.Size.Width + 10,
                    pinSelectionTop - 7.5f,
                    (float)(CorrectWidth - DigitsTextField.Frame.Size.Width) / 2 - 10,
                    30);
        ...
        private void LayoutButtonArea()
        {
            ...
            float topRowTop = (float)DetailLabel.Frame.Y + (float)DetailLabel.Frame.Size.Height + 15;

            if (!IsIphone5)
                topRowTop = (float)DetailLabel.Frame.Y + (float)DetailLabel.Frame.Size.Height + 10;
        ...
        {
            RectangleF newFrame = new RectangleF((float)roundedView.Frame.X,
                                      (float)roundedView.Frame.Y, newSize, newSize);
            ...
            else
            {
                if (BackgroundBlurringView == null)
                {
                    ...
                    else
                    {
                        ...
                        BackgroundBlurringView.BackgroundColor = UIColor.FromWhiteAlpha(0.8f, 0.1f);
                    }
        ...
        private async void PerformAnimations(Action animations, bool animated)
        {
            ...
            await UIView.AnimateAsync(length, animations);
        }
